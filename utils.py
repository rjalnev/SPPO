...
def Now(separate=True):
    now = datetime.now() #- timedelta(hours=6) #timedelata needed for Google Colab
    if separate:
...
# unstack and show observations
def show_observation(stacked_obs):
    fig, axes = plt.subplots(2,2,figsize=(12, 12), constrained_layout=True)
    axes = axes.flatten()
    for i in range(stacked_obs.shape[3]):
